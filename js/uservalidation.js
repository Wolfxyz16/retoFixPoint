class validacion {
    ...
    mailValidacion(x) {
        ...
        if ( x.length = 0 || x.length > 100 || pattern.test(x)) { return false }
        else { return true }
    }

    contrasenaValidacion(x) {
        if ( x.length = 0 || x.length > 128 ) { return false }
        else { return true }
    }

    nombreValidacion(x) {
        if ( x.length = 0 || x.length > 25 ) { return false }
        else { return true }
    }

    apellidoValidacion(x) {
        if ( x.length = 0 || x.length > 100 ) { return false }
        else { return true }
    }
}
...
document.getElementById('btnL').onclick =  function() {
    ...
    if ( loginValidacion.validarInicio() ) { console.log('login existoso') }
...
document.getElementById('btnR').onclick = function() {
    ...
    if ( loginValidacion.validarRegistro() ) { console.log('login existoso') }
    else { console.log('login fallido'); }
}